...
- Audit records for authentication, appointments, schedules, and blocked periods
- API and unit tests
...
Important operations write an `AuditRecord` in the same database transaction as
the operation: registration, successful login, booking, cancellation,
rescheduling, schedule changes, and blocked-period changes. Failed transactions
are rolled back together, so a rejected booking does not create a success audit
event. Audit details contain operation metadata only and never passwords or
secret values.
