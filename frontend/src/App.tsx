...
function App() {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand-mark">A<span>/</span>S</div><div className="side-title">Your day,<br /><strong>in balance.</strong></div><nav><a className="active">Overview</a><a>Appointments</a><a>Providers</a></nav><div className="side-foot"><div className="avatar">{user.name[0]}</div><div><strong>{user.name}</strong><small>{user.role.toLowerCase()}</small></div><button onClick={logout} title="Sign out">↗</button></div></aside><main className="dashboard"><header><div><p className="eyebrow">Thursday, September 24</p><h1>Good morning, {user.name.split(' ')[0]}.</h1></div><div className="status"><i /> System operational</div></header>{message && <div className="notice inline">{message}</div>}<section className="metric-row"><div><span>Upcoming</span><strong>{appointments.filter((a) => a.status === 'CONFIRMED').length.toString().padStart(2, '0')}</strong></div><div><span>Providers</span><strong>{providers.length.toString().padStart(2, '0')}</strong></div><div><span>Next focus</span><strong>{appointments[0] ? formatSlot(appointments[0].start_datetime) : '—'}</strong></div></section><section className="workspace"><div className="booking-panel"><div className="section-heading"><div><p className="eyebrow">Find a time</p><h2>Book an appointment</h2></div><input className="date-input" type="date" value={date} onChange={(e) => { setDate(e.target.value); if (selectedProvider) loadAvailability(selectedProvider) }} /></div><div className="provider-grid">{providers.map((provider) => <button className={`provider-card ${selectedProvider?.id === provider.id ? 'selected' : ''}`} key={provider.id} onClick={() => loadAvailability(provider)}><span className="provider-icon">{provider.name[0]}</span><span><strong>{provider.name}</strong><small>{provider.specialization || 'General provider'}</small></span><b>→</b></button>)}</div>{selectedProvider && <div className="slots"><div className="slot-heading"><span>Available with {selectedProvider.name}</span><small>{date}</small></div>{slots.length ? <div className="slot-grid">{slots.map((slot) => <button key={slot.start_datetime} onClick={() => book(slot)}>{formatSlot(slot.start_datetime)}<small>30 min</small></button>)}</div> : <p className="empty">No open slots for this date. Try another day.</p>}</div>}</div><div className="appointments-panel"><div className="section-heading"><div><p className="eyebrow">Your schedule</p><h2>Appointments</h2></div><span className="count">{appointments.length}</span></div>{appointments.length ? <div className="appointment-list">{appointments.map((appointment) => <article key={appointment.id}><div className="appointment-date"><strong>{new Date(appointment.start_datetime).getDate()}</strong><small>{new Intl.DateTimeFormat('en', { month: 'short' }).format(new Date(appointment.start_datetime))}</small></div><div className="appointment-info"><strong>{providers.find((p) => p.id === appointment.provider_id)?.name || `Provider #${appointment.provider_id}`}</strong><span>{formatSlot(appointment.start_datetime)} · {appointment.status.toLowerCase()}</span></div>{appointment.status === 'CONFIRMED' && <button onClick={() => cancel(appointment.id)} title="Cancel appointment">×</button>}</article>)}</div> : <div className="empty large">Your calendar is clear.<br /><span>Choose a provider to make your first booking.</span></div>}</div></section></main></div>
}
